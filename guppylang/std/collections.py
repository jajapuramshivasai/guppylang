...
@guppy
@no_type_check
def _bubble_up(self: PriorityQueue[T, MAX_SIZE] @ owned, index: int) -> PriorityQueue[T, MAX_SIZE]:
    ...
    if current_priority > parent_priority:
        ...
        return _bubble_up(self, parent_idx)
    
    return self

@guppy
@no_type_check  
def _bubble_down(self: PriorityQueue[T, MAX_SIZE] @ owned, index: int) -> PriorityQueue[T, MAX_SIZE]:
    """Maintains max-heap property by bubbling element at index downward."""
    left_idx = _left_child_index(index)
    right_idx = _right_child_index(index)
    largest_idx = index
    
    # Find the largest among current node and its children
    if left_idx < self.size:
        left_priority = self.buf[left_idx].unwrap()[0]
        current_priority = self.buf[largest_idx].unwrap()[0]
        if left_priority > current_priority:
            largest_idx = left_idx
    
    if right_idx < self.size:
        right_priority = self.buf[right_idx].unwrap()[0]
        largest_priority = self.buf[largest_idx].unwrap()[0]
        if right_priority > largest_priority:
    ...
    if largest_idx != index:
        ...
        return _bubble_down(self, largest_idx)
    
    return self

@guppy
@no_type_check
def push(self: PriorityQueue[T, MAX_SIZE] @ owned, value: T @ owned, priority: int) -> PriorityQueue[T, MAX_SIZE]:
    ...
    return _bubble_up(new_queue, self.size)

@guppy
@no_type_check
def pop(self: PriorityQueue[T, MAX_SIZE] @ owned) -> tuple[int, T, PriorityQueue[T, MAX_SIZE]]:
    ...
    return priority, value, _bubble_down(new_queue, 0)

@guppy
@no_type_check
def peek(self: PriorityQueue[TCopyable, MAX_SIZE] @ owned) -> tuple[int, TCopyable, PriorityQueue[TCopyable, MAX_SIZE]]:
